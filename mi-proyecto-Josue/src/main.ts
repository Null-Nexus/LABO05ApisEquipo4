...
interface Comment {
  postId: number;   // ID del post (numérico)
  id: number;       // ID único del post (numérico)
  name: string;    // Título del post (texto)
  email: string; //Email de quien hizo el post(texto)
  body: string;     // Contenido del post (texto)
}
/**
...
const fetchCommentsByPost = async (postId: number): Promise<void> => {
  ...
  try {
    ...
    if(!response.ok){
      throw new Error ("Error con la peticion de los comentarios");
    }
    ...
    // Tip: Usa data.length.
    console.log(`Total comentarios: ${data.length}`);
    ...
    console.error("🚩💀 Error, no se puede hacer la peticion de comentarios", error);
  }
};
...
const SUPABASE_KEY: string = "sb_publishable_3wy1WecMJSUAh3DFCyPVGA_-FNLIIAj";
 ...
 */
interface Salon {
  id_salon: number;       // Columna ID (Primary Key)
  status: boolean;       // Columna Patente (Varchar)
}
 ...
 */
const getSalon = async (): Promise<void> => {
