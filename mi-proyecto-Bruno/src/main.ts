...
const fetchCommentsByPost = async (postId: number): Promise<void> => {
  ...
  if (IS_DEBUG_MODE) {
    console.log(`%c [LOG] Buscando comentarios del post con ID: ${postId}...`, "color: cyan; font-weight: bold;");
  }
  ...
  try {
    ...
    console.log(`Al post llegaron ${data.length} comentarios`);
 ...
 */
const getTutor = async (): Promise<void> => {
